...
public class EncerraQuartoController {
    ...
    public void salvarLocacao(List<vendaProdutos> produtosVendidos, float pagoDinheiro, float pagoPix,
        ...
        // 2. Salvar Cartão Se Houver
        if (pagoCredito > 0 || pagoDebito > 0) {
            salvaCartao(pagoCredito, pagoDebito);
        }
    ...
    private void salvaCartao(float recebidoCredito, float recebidoDebito) {
        ...
        try (Connection link = new fazconexao().conectar();
            ...
            logger.error("Erro ao salvar cartão: ", e);
        }
    }
