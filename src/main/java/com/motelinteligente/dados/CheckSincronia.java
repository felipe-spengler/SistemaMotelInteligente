...
public class CheckSincronia extends TimerTask {
        ...
        try (PreparedStatement sl = local.prepareStatement(sqlSelectLocal)) {
            ...
            try (ResultSet rl = sl.executeQuery()) {
                if (rl.next()) {
                    ...
                    // 1.1 Descobre colunas de ambos os bancos para evitar erro de schema desalinhado
                    List<String> remoteColumns = getColumnNames(tabela, remoto);
                    List<String> localColumns = getColumnNames(tabela, local);
                    
                    // Intersecção: apenas colunas que existem em AMBOS
                    ...
                    if (columns.isEmpty()) {
                        logger.warn("Tabela " + tabela + " não possui colunas compatíveis para sincronização.");
                        return;
                    }
    ...
    // Método auxiliar para obter nomes das colunas (Case-Insensitive)
    private List<String> getColumnNames(String tabela, Connection conn) throws SQLException {
        List<String> columns = new ArrayList<>();
